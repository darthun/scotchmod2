...
import net.darthun.scotchmod.block.entity.MashTunBlockEntity;
...
public class MashTunBlockEntityRenderer implements BlockEntityRenderer<MashTunBlockEntity> {
    public MashTunBlockEntityRenderer(BlockEntityRendererProvider.Context context){

    }
    @Override
    public void render(MashTunBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer,
        ...
        if(!fluidStack.isEmpty()){
            pPoseStack.pushPose();
            this.renderSingleBlock(blockRenderer, ModBlocks.CUSTOM_WATER.get().defaultBlockState(),pPoseStack,pBuffer,pPackedLight,pPackedOverlay,ModelData.EMPTY,RenderType.translucent(),true);
            pPoseStack.popPose();
        }
        else{
            if (!pBlockEntity.getOutputFluid().isEmpty()){
                pPoseStack.pushPose();
                this.renderSingleBlock(blockRenderer, ModBlocks.CUSTOM_WATER.get().defaultBlockState(),pPoseStack,pBuffer,pPackedLight,pPackedOverlay,ModelData.EMPTY,RenderType.translucent(),false);
                pPoseStack.popPose();
            }
        ...
        if(itemStack.getItem() != ModBlocks.SMOKED_MALTED_BARLEY_BLOCK.get().asItem()){
            pPoseStack.scale(0.5f,0.5f,0.5f);
        }
        pPoseStack.mulPose(Axis.YN.rotationDegrees(pBlockEntity.getBlockState().getValue(MashTunBlock.FACING).toYRot()));
...
//    }
    ...
    public void renderSingleBlock(BlockRenderDispatcher pDispatcher,BlockState pState, PoseStack pPoseStack, MultiBufferSource pBufferSource, int pPackedLight, int pPackedOverlay, ModelData modelData, RenderType renderType, boolean isInputFluid) {
        RenderShape rendershape = pState.getRenderShape();
        //int water  = 0x3F76E4
        //int wort = 0xFF864B09
        int i = isInputFluid ? 0x3F76E4:0xFF864B09;
